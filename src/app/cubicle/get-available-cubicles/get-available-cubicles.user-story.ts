...
export class GetAvailableCubiclesUserStory{
  ...
  async execute(
    input: GetAvailableCubiclesUserStoryInput): Promise<GetAvailableCubiclesUserStoryOutput[]> {
      ...
      const availableCubicles = campusCubicles.filter(
        (c)=> c.reservations.find(
          ((r) => (new Date(r.startTime) === input.startHourDatetime &&
          (new Date(r.endTime) === inputEndDatetime).toString()) ||
           (new Date(r.startTime) === input.startHourDatetime && (new Date(r.startTime) === inputMidDatetime).toString()))
      ) == null
    );
